...
- `A_Shop.dc.html` — the artboard (a Design Component: an `<x-dc>`
  template + a small logic class). The values to replicate live in its
  inline `style="…"` attributes and the `<helmet><style>` block.
- `amp-main.jpg` — referenced resource
- `wollensak-straight.jpg` — referenced resource
- `akai-hero.jpg` — referenced resource
- `support.js`, `vendor/react*.js` — the runtime that renders the
  component in a browser; not part of the design.

## Viewing

Serve the folder (e.g. `python3 -m http.server`) and open `A_Shop.dc.html`;
